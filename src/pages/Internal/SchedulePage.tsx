...
export default function SchedulePage() {
  ...
  useEffect(() => {
    // `date` is an ISO date (YYYY-MM-DD) picked in local time; parse as local and compute next-day boundary.
    const start = new Date(`${date}T00:00:00`);
    const end   = new Date(start.getTime() + 24 * 60 * 60 * 1000);
    internalApi.getSchedule(start.toISOString(), end.toISOString(), staffFilter || undefined)
  ...
  async function changeStatus(id: number, code: AppointmentDto['statusCode']) {
    ...
    setAppointments((list) => list.map((a) =>
      a.appointmentId === id ? { ...a, statusCode: code } : a));
  }
  ...
  return (
    <div>
      ...
      {byTime.map((a) => {
        ...
        return (
          <div key={a.appointmentId} className="sched-row">
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {a.statusCode === 'new' && <button className="btn btn-sm btn-success" onClick={() => changeStatus(a.appointmentId, 'confirmed')}>{t('sched.confirm', 'Confirm')}</button>}
              {(a.statusCode === 'new' || a.statusCode === 'confirmed') && <button className="btn btn-sm btn-primary" onClick={() => changeStatus(a.appointmentId, 'completed')}>{t('sched.complete', 'Complete')}</button>}
              {a.statusCode !== 'cancelled' && a.statusCode !== 'completed' && a.statusCode !== 'no_show' &&
                <button className="btn btn-sm btn-outline-danger" onClick={() => changeStatus(a.appointmentId, 'cancelled')}>{t('sched.cancel', 'Cancel')}</button>}
            </div>
          </div>
        );
      })}
